...
const WatchLecture = () => {
  ...
  useEffect(() => {
    const fetchLectureData = async () => {
      try {
        ...
        if (
          !response.data ||
          !Array.isArray(response.data.lectures) ||
        ...
        if (
          isNaN(validIndex) ||
          validIndex < 0 ||
          validIndex >= response.data.lectures.length
        ) {
          validIndex = 0;
        }
  ...
  useEffect(() => {
    if (
      ...
      currentLectureIndex < moduleLectures.length
    ) {
  ...
  const handleNext = () => {
    if (currentLectureIndex < moduleLectures.length - 1) {
      navigate(
        `/admin/assets/courses/watch-lecture/${courseId}/${moduleId}/${
